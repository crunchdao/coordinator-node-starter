...
6. Run `make test-e2e` to validate the full stack end-to-end.
...
- Resolution: `CRUNCH_CONFIG_MODULE` env → `config.crunch_config:CrunchConfig` → engine default
- Operator's config in `node/config/crunch_config.py` is auto-loaded (on PYTHONPATH in Docker)
