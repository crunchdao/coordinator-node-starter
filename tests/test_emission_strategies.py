...
class TestContributionWeightedEmission(unittest.TestCase):
    ...
    def test_rewards_sum_to_frac64(self):
        entries = self._make_entries([
            {"rank": 1, "summary": {"contribution": 0.05, "model_correlation": 0.2}},
            {"rank": 2, "summary": {"contribution": 0.02, "model_correlation": 0.8}},
            {"rank": 3, "summary": {"contribution": -0.01, "model_correlation": 0.5}},
        ])
    ...
    def test_high_contribution_gets_more(self):
        entries = self._make_entries([
            {"rank": 1, "summary": {"contribution": 0.10, "model_correlation": 0.1}},
            {"rank": 2, "summary": {"contribution": 0.00, "model_correlation": 0.9}},
        ])
        emission = contribution_weighted_emission(entries, "crunch123")
    ...
    def test_diverse_model_gets_more(self):
        ...
        entries = self._make_entries([
            {"rank": 1, "summary": {"contribution": 0.02, "model_correlation": 0.1}},
            {"rank": 2, "summary": {"contribution": 0.02, "model_correlation": 0.9}},
        ])
        emission = contribution_weighted_emission(
            entries, "crunch123",
            rank_weight=0.0, contribution_weight=0.0, diversity_weight=1.0,
        )
    ...
    def test_min_pct_floor(self):
        entries = self._make_entries([
            ...
            {"rank": 2, "summary": {"contribution": -1.0, "model_correlation": 1.0}},
        ])
        ...
        min_frac = int(5.0 / 100.0 * FRAC_64_MULTIPLIER * 0.9)  # ~4.5% accounting for renorm
    ...
    def test_providers_passed_through(self):
        ...
        self.assertEqual(emission["compute_provider_rewards"][0]["provider"], "cp_wallet")
