...
class TestTickSubjectKeying:
    """tick() must store data per-subject so multi-asset works."""

    def test_stores_data_by_symbol(self):
        tracker = DummyTracker()
        tracker.tick({"symbol": "BTC", "price": 100})
        tracker.tick({"symbol": "ETH", "price": 50})
    ...
    def test_does_not_overwrite_other_subjects(self):
        ...
        tracker.tick({"symbol": "BTC", "price": 100})
        tracker.tick({"symbol": "ETH", "price": 50})
        tracker.tick({"symbol": "ETH", "price": 55})
    ...
    def test_updates_same_subject(self):
        tracker = DummyTracker()
        tracker.tick({"symbol": "BTC", "price": 100})
        tracker.tick({"symbol": "BTC", "price": 200})
...
class TestGetDataFallback:
    ...
    def test_falls_back_to_default_when_no_symbol(self):
        tracker = DummyTracker()
        tracker.tick({"price": 100})  # no symbol key
    ...
    def test_exact_match_takes_priority_over_default(self):
        tracker = DummyTracker()
        tracker.tick({"price": 999})  # stored as _default
        tracker.tick({"symbol": "BTC", "price": 100})
