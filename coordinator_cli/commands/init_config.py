...
from coordinator_cli.commands.packs import get_pack
...
def _validate_callable_path(key: str, value: str) -> None:
    ...
    if module_path.startswith("coordinator"):
        raise ValueError(
            f"Callable '{key}' uses internal module path '{module_path}'. "
            "Use coordinator.feeds.* or challenge/node-local module paths instead."
        )
